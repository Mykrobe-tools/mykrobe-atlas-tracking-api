...
def test_add_or_update_qc_result_of_non_existent_sample(sample_id, qc_result, create_or_replace_qc_result):
    response = create_or_replace_qc_result(sample_id, qc_result)
    assert response.status_code == 404, response.data.decode()


@given(sample_id=sample_ids(), qc_result=qc_results())
def test_add_qc_result(sample_id, qc_result, create_sample, create_or_replace_qc_result, get_qc_result):
    with managed_db():
...
def test_replace_qc_result(sample_id, original, new, create_sample, create_or_replace_qc_result, get_qc_result):
    with managed_db():
        create_sample(sample_id)
        create_or_replace_qc_result(sample_id, original, ensure=True)

        response = create_or_replace_qc_result(sample_id, new)
...
@given(sample_id=sample_ids(), qc_result=qc_results())
def test_getting_qc_results(sample_id, qc_result, create_sample, create_or_replace_qc_result, get_qc_result):
    with managed_db():
        create_sample(sample_id)
        create_or_replace_qc_result(sample_id, qc_result, ensure=True)
...
@given(sample_id=sample_ids(), qc_result=qc_results())
def test_deleting_files(sample_id, qc_result, create_sample, create_or_replace_qc_result, delete_qc_result, get_qc_result):
    with managed_db():
        create_sample(sample_id)
        create_or_replace_qc_result(sample_id, qc_result, ensure=True)
